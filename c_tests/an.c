...
unsigned long getms( timeval * start, timeval * end )
{
    unsigned long long s = start->tv_sec * 1000000 + start->tv_usec;
    unsigned long long e = end->tv_sec * 1000000 + end->tv_usec;
    return (unsigned long) ( ( e - s ) / 1000 );
}
...
extern "C" int main( int argc, char * argv[] )
{
    ...
    try
    {
        ...
        timeval tvAfterReadWords;
        gettimeofday( &tvAfterReadWords, 0 );
        ...
        timeval tvAfterGenerate;
        gettimeofday( &tvAfterGenerate, 0 );
        ...
        if ( showTime )
        {
            timeval tvAfterRender;
            gettimeofday( &tvAfterRender, 0 );
            high_resolution_clock::time_point tAfterRender = high_resolution_clock::now();

#if 1 // old gcc chrono only has 1-second resolution!

            printf( "total ms:     %u\n", getms( &tvStart, &tvAfterRender ) );
            printf( "  dictionary: %u\n", getms( &tvStart, &tvAfterReadWords ) );
            printf( "  generate:   %u\n", getms( &tvAfterReadWords, &tvAfterGenerate ) );
            printf( "  render:     %u\n", getms( &tvAfterGenerate, &tvAfterRender ) );

#else

            long totalTime = duration_cast<std::chrono::milliseconds>( tAfterRender - tStart ).count();
            long dictionaryTime = duration_cast<std::chrono::milliseconds>( tAfterReadWords - tStart ).count();
            long generateTime = duration_cast<std::chrono::milliseconds>( tAfterGenerate - tAfterReadWords ).count();
            long renderTime = duration_cast<std::chrono::milliseconds>( tAfterRender - tAfterGenerate ).count();
